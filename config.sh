# litellm-proxy — client-side model switcher for Claude Code (bash/zsh).
#
# Source this from your ~/.bashrc or ~/.zshrc, then pick a model:
#
#   source $HOME/litellm-proxy/config.sh   # defines `usemodel`, loads keys from .env
...
if [ -n "${BASH_SOURCE:-}" ]; then
    _litellm_self="${BASH_SOURCE[0]}"
elif [ -n "${ZSH_VERSION:-}" ]; then
    _litellm_self="${(%):-%x}"
else
    _litellm_self="$0"
fi
. "$(dirname "$_litellm_self")/lib/usemodel.sh"
